---
tag          : vessel_Blues
mission      : Special Event
origin_locn  : Camaret
origin_moor  : marina
via          : Pentrez
via_moor     : anchor
destination  : 
terminus_locn: Camaret 
terminus_moor: anchor
distance_planned_NM: 
distance_traveld_NM: 70
offshore_percentage:  
duration_h         : 19
max_speed          :  4.6
conditions   : light to moderate
crew         : Carole
skipper      : Boran
gps_track    : https://connect.garmin.com/modern/activity/19167773812
photos       : 
---
Special trip with Carole.

Anchored over night. Visited Kira, Isa & Flo next morning.

Sailed without engine because the old one was defective and the new one has not arrived yet.

Attempted to anchor in 70m on way back, near Cap de la Chevre, because of total calm.

Challenging anchor approach at destination: moderate breeze from North, making final approach to the West coast fast, and pushing the boat South (towards quai) after stopping. No engines for corrections. This is almost like an airplane approach with only one attempt.

## Incidents

1. During the approach to Pentrez, the wind picked up to a fresh breeze. It flipped the SUP around which had been inflated and strapped to the side of the starboard hull. Had to recover and deflate the SUP while sailing.

1. The turnbuckle of the starboard-rear shroud disconnected. Luckily no part was lost, and I could connect it while sailing.

## Lessons learned

1. Deflate and store SUP below deck while sailing.

1. Check all turnbuckles on all shrouds every time in the take-off procedure (it is part of the checklist)
